...
const CategoryCard = ({ category, onSelect, progress, score }: CategoryCardProps) => {
  return (
    <div
    ...
    >
      ...
      <div className="mt-4">
        <div className="flex justify-between items-center mb-1 text-sm font-medium text-gray-600 dark:text-gray-300">
            <span>پیشرفت</span>
            <span>امتیاز: <span className="font-bold text-teal-500">{score}</span></span>
        </div>
        <ProgressBar progress={progress} />
        <div className="text-right text-xs mt-1.5 text-gray-400 dark:text-gray-500">{Math.round(progress)}% کامل شده</div>
      </div>
    </div>
  );
};
